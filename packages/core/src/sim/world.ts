...
export function advanceDay(state: WorldState, map: WorldMap, rng: Rng, passedOut: boolean): { summary: DaySummary; changed: number[] } {
  ...
  for (const key of Object.keys(state.soil)) {
    ...
    const y = (k - x) / map.w;
    if (s.crop) {
      const r = growCropOneDay(s, ended, isCovered(state, x, y));
  ...
  return {
    summary: { day: state.clock.day, shipment, deaths, ready, weather: state.weather, forecast: state.forecast, passedOut },
